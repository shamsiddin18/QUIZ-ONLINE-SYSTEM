    ...
    foreach($_POST as $key => $value) {
        if(strpos($key, 'question_answer_') !== false){
            ...
            if(!empty($_FILES['fileToUpload_'.$questionNumber]['name'])) {
                $imagePath = '/quizproject/assets/src/images/'.$_FILES['fileToUpload_'.$questionNumber]['name'];
            ...
            if(!$result) {
                ...
                header("Location: http://localhost/quizproject/quizhomepage.php?message='$message'");
                exit();                
            }
